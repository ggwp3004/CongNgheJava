...
public class HoaDonDAL {
    ...
    public List<HoaDon> findAll() throws SQLException {
        ...
        String sql = "SELECT hd.ma_hd, hd.ngay_lap, hd.ma_kh, hd.tong_tien, kh.ten_kh " +
                "FROM hoa_don hd JOIN khach_hang kh ON hd.ma_kh = kh.ma_kh ORDER BY hd.ma_hd DESC";
    ...
    public List<ChiTietHoaDon> findChiTietByHoaDon(int maHd) throws SQLException {
        ...
        String sql = "SELECT ct.ma_sp, sp.ten_sp, ct.so_luong, ct.don_gia, ct.thanh_tien " +
                "FROM chi_tiet_hoa_don ct JOIN san_pham sp ON ct.ma_sp = sp.ma_sp WHERE ct.ma_hd = ?";
        try (Connection conn = DBHelper.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, maHd);
    ...
    /**
     * Lưu hóa đơn + chi tiết hóa đơn + trừ kho trong CÙNG 1 transaction.
     * Nếu bất kỳ bước nào lỗi (VD không đủ tồn kho) -> rollback toàn bộ.
     */
    public int insertHoaDon(int maKh, List<ChiTietHoaDon> chiTietList) throws SQLException {
        ...
        try {
            ...
            // Trừ kho cho từng sản phẩm trong cùng transaction
            SanPhamDAL sanPhamDAL = new SanPhamDAL();
            for (ChiTietHoaDon ct : chiTietList) {
                sanPhamDAL.truKho(conn, ct.getMaSp(), ct.getSoLuong());
            }
