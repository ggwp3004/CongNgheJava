...
public class SanPhamDAL {
    ...
    public SanPham findById(int maSp) throws SQLException {
        String sql = "SELECT ma_sp, ten_sp, don_gia, so_luong FROM san_pham WHERE ma_sp = ?";
        try (Connection conn = DBHelper.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, maSp);
    ...
    /** Trừ kho sau khi bán (dùng trong cùng 1 transaction ở HoaDonDAL). */
    public void truKho(Connection conn, int maSp, int soLuongBan) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            if (rows == 0) {
                throw new SQLException("Khong du ton kho cho san pham ma = " + maSp);
            }
        }
    }
